...
define(['extm', 'marionette'], function(Extm, Marionette) {
  ...
  UnitView = (function(_super) {
    ...
    UnitView.prototype.template = '<div id="{{id}}" class="boxBlank"> <div class="pull-left light text-center"> <h4 class="rangeName bold m-t-5"></h4> <div class="small">{{name}}</div> </div> <div class="unitCount pull-right text-center"> <h4 class="bold m-b-5 m-t-0">{{count}}</h4> <div class="small">FLATS</div> </div> <div class="clearfix"></div> <input type="hidden" name="checkrange{id}}"   id="checkrange{{id}}"   value="0" /> </div>';

    UnitView.prototype.events = {
      'click ': function(e) {
        var buildingModel, element, floorriserange, index, q, _i, _len;
        q = 1;
        $.map(App.backFilter, function(value, index) {
          var element, screenArray, _i, _len;
          if (q !== 1) {
        ...
        if (this.model.get('count') !== 0) {
          for (index = _i = 0, _len = rangeArray.length; _i < _len; index = ++_i) {
            ...
            if (element === this.model.get('id')) {
              $("#checkrange" + this.model.get('id')).val('1');
            } else {
          ...
          if (parseInt($("#checkrange" + this.model.get('id')).val()) === 0) {
            rangeArray.push(this.model.get('range') + this.model.get('id'));
            $('#' + this.model.get('id')).addClass('selected');
            $("#checkrange" + this.model.get('id')).val("1");
            buildingModel = App.currentStore.building.findWhere({
              id: this.model.get('id')
            });
            floorriserange = buildingModel.get('floorriserange');
            App.defaults['building'] = parseInt(this.model.get('id'));
            ...
            rangeArray = [];
            $("#checkrange" + this.model.get('id')).val("0");
            $('#' + this.model.get('id')).removeClass('selected');
          }
        }
        if (parseInt($("#checkrange" + this.model.get('id')).val()) === 0) {
